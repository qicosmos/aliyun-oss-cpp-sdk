...
#ifdef USE_CPP20
async_simple::coro::Lazy<void> BucketSample::test_coro_lazy(
    const AlibabaCloud::OSS::CreateBucketRequest &request) {
  co_await client->CreateBucketAsync(request);
}
#endif

BucketSample::BucketSample(const std::string &bucket) : bucket_(bucket) {
  ClientConfiguration conf;

  client = new OssClient(Config::Endpoint, Config::AccessKeyId,
                         Config::AccessKeySecret, conf);
  CreateBucketRequest request(bucket_);

  // #ifdef USE_CPP20
  //   async_simple::coro::syncAwait(test_coro_lazy(request));
  // #else
  client->CreateBucket(request);
  // #endif
}

BucketSample::~BucketSample() { delete client; }

void BucketSample::PrintError(const std::string &funcName,
...
void BucketSample::InvalidBucketName() {
  SetBucketAclRequest request("invalid-BucketName",
  ...
  SetBucketLoggingRequest log_request("invalid-BucketName", bucket_,
                                      "LogPrefix");
...
void BucketSample::CreateAndDeleteBucket() {
  ...
  CreateBucketRequest request(bucket, StorageClass::IA,
...
void BucketSample::SetBucketLogging() {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, request_id:" << outcome.result().RequestId().c_str()
              << std::endl;
  } else {
    PrintError(__FUNCTION__, outcome.error());
  }
...
void BucketSample::SetBucketWebsite() {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, request_id:" << outcome.result().RequestId()
              << std::endl;
  } else {
    PrintError(__FUNCTION__, outcome.error());
  }
...
void BucketSample::GetBucketAcl() {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__ << " success, ori acl: " << outcome.result().Acl()
              << std::endl;
  } else {
    PrintError(__FUNCTION__, outcome.error());
  }
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__ << " success, after set public-read, acl:"
              << outcome.result().Acl() << std::endl;
  } else {
    PrintError(__FUNCTION__, outcome.error());
  }
}
...
void BucketSample::GetBucketLocation() {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, location: " << outcome.result().Location()
              << std::endl;
  } else {
    PrintError(__FUNCTION__, outcome.error());
  }
...
void BucketSample::GetBucketLogging() {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__ << " success, default TargetBucket: "
              << outcome.result().TargetBucket()
              << ",TargetPrefix: " << outcome.result().TargetPrefix()
              << std::endl;
  } else {
    PrintError(__FUNCTION__, outcome.error());
  }
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, after set itself with LogPrefix-00 , TargetBucket: "
              << outcome.result().TargetBucket()
              << " ,TargetPrefix: " << outcome.result().TargetPrefix()
              << std::endl;
  } else {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, after set other with LogPrefix-01 TargetBucket: "
              << outcome.result().TargetBucket()
              << " ,TargetPrefix: " << outcome.result().TargetPrefix()
              << std::endl;
  } else {
...
void BucketSample::GetBucketWebsite() {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__ << " success, default, IndexDocument: "
              << outcome.result().IndexDocument()
              << " ,ErrorDocument: " << outcome.result().ErrorDocument()
              << std::endl;
  } else {
    PrintError(__FUNCTION__, outcome.error());
  }
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, after set index.html, IndexDocument: "
              << outcome.result().IndexDocument()
              << " ,ErrorDocument: " << outcome.result().ErrorDocument()
              << std::endl;
  } else {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, after set index1.html, error1.html,IndexDocument: "
              << outcome.result().IndexDocument()
              << " ,ErrorDocument: " << outcome.result().ErrorDocument()
              << std::endl;
  } else {
...
void BucketSample::GetBucketReferer() {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__ << " success, deffault AllowEmptyReferer: "
              << outcome.result().AllowEmptyReferer()
              << " ,Referer size: " << outcome.result().RefererList().size()
              << std::endl;
  } else {
    PrintError(__FUNCTION__, outcome.error());
  }
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, after set 4 refer, AllowEmptyReferer: "
              << outcome.result().AllowEmptyReferer()
              << " ,Referer size: " << outcome.result().RefererList().size()
              << std::endl;
  } else {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, after set 2 refer, AllowEmptyReferer: "
              << outcome.result().AllowEmptyReferer()
              << " ,Referer size: " << outcome.result().RefererList().size()
              << std::endl;
  } else {
...
void BucketSample::GetBucketLifecycle() {
  ...
  if (outcome.isSuccess()) {
    ...
    for (auto const &rule : outcome.result().LifecycleRules()) {
      std::cout << "rule:" << rule.ID() << "," << rule.Prefix() << ","
                << rule.Status()
                << ","
                   "hasExpiration:"
                << rule.hasExpiration() << ","
                << "hasTransitionList:" << rule.hasTransitionList() << ","
                << "hasAbortMultipartUpload:" << rule.hasAbortMultipartUpload()
                << std::endl;
    }
  } else {
    PrintError(__FUNCTION__, outcome.error());
...
void BucketSample::GetBucketStat() {
  ...
  if (outcome.isSuccess()) {
    std::cout << __FUNCTION__
              << " success, storage: " << outcome.result().Storage()
              << " ,ObjectCount: " << outcome.result().ObjectCount()
              << " ,MultipartUploadCount:"
              << outcome.result().MultipartUploadCount() << std::endl;
  } else {
    PrintError(__FUNCTION__, outcome.error());
  }
}
...
void BucketSample::CleanAndDeleteBucket(const std::string &bucket) {
  ...
  if (infoOutcome.isSuccess() &&
      infoOutcome.result().VersioningStatus() != VersioningStatus::NotSet) {
    ...
    do {
      ...
      if (outcome.isSuccess()) {
        for (auto const &marker : outcome.result().DeleteMarkerSummarys()) {
          client->DeleteObject(
              DeleteObjectRequest(bucket, marker.Key(), marker.VersionId()));
        }

        for (auto const &obj : outcome.result().ObjectVersionSummarys()) {
          client->DeleteObject(
              DeleteObjectRequest(bucket, obj.Key(), obj.VersionId()));
        }
      } else {
        break;
      }
  ...
  auto listOutcome =
      client->ListMultipartUploads(ListMultipartUploadsRequest(bucket));
  if (listOutcome.isSuccess()) {
    for (auto const &upload : listOutcome.result().MultipartUploadList()) {
      client->AbortMultipartUpload(
          AbortMultipartUploadRequest(bucket, upload.Key, upload.UploadId));
    }
  }
  ...
  do {
    ...
    if (outcome.isSuccess()) {
      for (auto const &obj : outcome.result().ObjectSummarys()) {
        auto dOutcome =
            client->DeleteObject(DeleteObjectRequest(bucket, obj.Key()));
...
void BucketSample::DeleteBucketsByPrefix() {
  ...
  std::cout << __FUNCTION__ << " done, and total is "
            << outcome.result().Buckets().size() << std::endl;
}

void BucketSample::DoesBucketExist() {
  auto outcome = client->DoesBucketExist(bucket_);
  std::cout << __FUNCTION__ << " success, Bucket exist ? " << outcome
            << std::endl;
}