...
public:
  SelectObjectStreamBuf(std::iostream &stream, int initCrc32)
      : StreamBufProxy(stream), lastStatus_(0) {
    // init frame
  ...
  int LastStatus() { return lastStatus_; }

protected:
  int selectObjectDepackFrame(const char *ptr, int len, int *frame_type,
                              int *payload_len, char **payload_buf,
                              SelectObjectFrame *frame) {
    int remain = len;
    // Version | Frame - Type | Payload Length | Header Checksum | Payload |
    // Payload Checksum <1 byte> <--3 bytes-->   <-- 4 bytes --> <------4
    //bytes--> <variable><----4bytes------> Payload <offset | data> <8
    //types><variable>
    ...
    if (frame->header_len < FRAME_HEADER_LEN) {
      ...
      if (frame->header_len == FRAME_HEADER_LEN) {
        ...
        frame->payload_crc32 =
            CRC32::CalcCRC(frame->init_crc32, frame->header + 12, 8);
      }
    }
    ...
    if (frame->payload_remains > 0) {
      int copy =
          (frame->payload_remains > remain) ? remain : frame->payload_remains;
      uint32_t type;
      type = frame->header[1];
  ...
  int selectObjectTransferContent(SelectObjectFrame *frame, const char *ptr,
                                  int wanted) {
    int remain = wanted;
    char *payload_buf;
    ...
    while (remain > 0) {
      ...
      int ret = selectObjectDepackFrame(ptr, remain, &frame_type, &payload_len,
                                        &payload_buf, frame);
      switch (frame_type) {
      case 0x800001:
        int temp;
        temp =
            static_cast<int>(StreamBufProxy::xsputn(payload_buf, payload_len));
        if (temp < 0) {
          return temp;
        }
        result += temp;
  ...
  std::streamsize xsputn(const char *ptr, std::streamsize count) {
    int result =
        selectObjectTransferContent(&frame_, ptr, static_cast<int>(count));
    if (result < 0) {
      if (result == -1) {
...
SelectObjectRequest::SelectObjectRequest(const std::string &bucket,
                                         const std::string &key)
    : GetObjectRequest(bucket, key), expressionType_(ExpressionType::SQL),
      skipPartialDataRecord_(false), maxSkippedRecordsAllowed_(0),
      inputFormat_(nullptr), outputFormat_(nullptr), streamBuffer_(nullptr),
...
void SelectObjectRequest::setResponseStreamFactory(
    const IOStreamFactory &factory) {
  ...
  ServiceRequest::setResponseStreamFactory([this]() {
    ...
    if (!outputFormat_->OutputRawData()) {
      ...
      streamBuffer_ =
          std::make_shared<SelectObjectStreamBuf>(*content, initCrc32);
    }
...
void SelectObjectRequest::setSkippedRecords(bool skipPartialDataRecord,
                                            uint64_t maxSkippedRecords) {
  skipPartialDataRecord_ = skipPartialDataRecord;
  maxSkippedRecordsAllowed_ = maxSkippedRecords;
}

void SelectObjectRequest::setExpression(const std::string &expression,
                                        ExpressionType type) {
...
std::string SelectObjectRequest::payload() const {
  ...
  // Expression
  ss << "<Expression>" << Base64Encode(expression_) << "</Expression>"
     << std::endl;
  // input format
  ...
  // options
  ss << "<Options>" << std::endl;
  ss << "<SkipPartialDataRecord>" << (skipPartialDataRecord_ ? "true" : "false")
     << "</SkipPartialDataRecord>" << std::endl;
  ss << "<MaxSkippedRecordsAllowed>"
     << std::to_string(MaxSkippedRecordsAllowed())
     << "</MaxSkippedRecordsAllowed>" << std::endl;
  ss << "</Options>" << std::endl;
  ss << "</SelectRequest>" << std::endl;
  return ss.str();
}
