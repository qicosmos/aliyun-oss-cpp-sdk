...
public:
  ...
  Client(const std::string &servicename,
...
protected:
#ifdef USE_CPP20
  async_simple::coro::Lazy<ClientOutcome>
  AttemptRequestAsync(const std::string &endpoint,
                      const ServiceRequest &request, Http::Method method) const;
  async_simple::coro::Lazy<ClientOutcome>
  AttemptOnceRequestAsync(const std::string &endpoint,
                          const ServiceRequest &request,
                          Http::Method method) const;
#endif

  ClientOutcome AttemptRequest(const std::string &endpoint,
                               const ServiceRequest &request,
                               Http::Method method) const;
  ClientOutcome AttemptOnceRequest(const std::string &endpoint,
                                   const ServiceRequest &request,
                                   Http::Method method) const;
  virtual std::shared_ptr<HttpRequest>
  buildHttpRequest(const std::string &endpoint, const ServiceRequest &msg,
                   Http::Method method) const = 0;
  virtual bool
