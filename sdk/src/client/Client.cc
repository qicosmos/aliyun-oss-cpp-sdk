...
Client::Client(const std::string &servicename,
...
std::string Client::serviceName() const { return serviceName_; }

Client::ClientOutcome Client::AttemptRequest(const std::string &endpoint,
  ...
  for (int retry = 0;; retry++) {
    ...
    if (outcome.isSuccess()) {
      ...
      if (retryStrategy == nullptr ||
          !retryStrategy->shouldRetry(outcome.error(), retry)) {
        return outcome;
      }
...
Client::ClientOutcome Client::AttemptOnceRequest(const std::string &endpoint,
                                                 const ServiceRequest &request,
                                                 Http::Method method) const {
  if (!httpClient_->isEnable()) {
    return ClientOutcome(
        Error("ClientError:100002", "Disable all requests by upper."));
  }
...
Client::AttemptRequestAsync(const std::string &endpoint,
                            const ServiceRequest &request,
                            Http::Method method) const {
  for (int retry = 0;; retry++) {
    auto outcome = co_await AttemptOnceRequestAsync(endpoint, request, method);
    if (outcome.isSuccess()) {
      co_return outcome;
    } else if (!httpClient_->isEnable()) {
      co_return outcome;
...
async_simple::coro::Lazy<Client::ClientOutcome>
Client::AttemptOnceRequestAsync(const std::string &endpoint,
                                const ServiceRequest &request,
                                Http::Method method) const {
  if (!httpClient_->isEnable()) {
    co_return ClientOutcome(
        Error("ClientError:100002", "Disable all requests by upper."));
  }
...
Error Client::buildError(const std::shared_ptr<HttpResponse> &response) const {
  ...
  std::stringstream ss;
  if ((responseCode == 203) || (responseCode > 299 && responseCode < 600)) {
...
void Client::disableRequest() { httpClient_->disable(); }

void Client::enableRequest() { httpClient_->enable(); }

bool Client::isEnableRequest() const { return httpClient_->isEnable(); }

void Client::setRequestDateOffset(uint64_t offset) const {
  requestDateOffset_ = offset;
}

uint64_t Client::getRequestDateOffset() const { return requestDateOffset_; }