...
GetObjectOutcome ResumableDownloader::Download() {
  ...
  for (uint32_t i = 0; i < request_.ThreadNum(); i++) {
    threadPool.emplace_back(std::thread([&]() {
      ...
      while (true) {
        ...
        auto getObjectReq =
            GetObjectRequest(request_.Bucket(), request_.Key(),
                             request_.ModifiedSinceConstraint(),
                             request_.UnmodifiedSinceConstraint(),
                             request_.MatchingETagsConstraint(),
                             request_.NonmatchingETagsConstraint(),
                             request_.ResponseHeaderParameters());
        getObjectReq.setResponseStreamFactory([this, pos]() {
          auto tmpFstream = GetFstreamByPath(
              request_.TempFilePath(), request_.TempFilePathW(),
              std::ios_base::in | std::ios_base::out | std::ios_base::binary);
          tmpFstream->seekp(pos, tmpFstream->beg);
          return tmpFstream;
        });
        getObjectReq.setRange(start, end);
        getObjectReq.setFlags(getObjectReq.Flags() | REQUEST_FLAG_CHECK_CRC64 |
                              REQUEST_FLAG_SAVE_CLIENT_CRC64);
        ...
        if (process.Handler) {
          ...
          TransferProgress uploadPartProcess = {DownloadPartProcessCallback,
                                                (void *)&transferState};
        ...
        // lock
        {
          ...
          if (outcome.isSuccess()) {
            part.crc64 = std::strtoull(outcome.result()
                                           .Metadata()
                                           .HttpMetaData()
                                           .at("x-oss-hash-crc64ecma-by-client")
                                           .c_str(),
                                       nullptr, 10);
          ...
          if (hasRecordPath() && outcome.isSuccess()) {
            ...
            auto recordStream =
                GetFstreamByPath(recordPath_, recordPathW_, std::ios::out);
  ...
  if (!client_->isEnableRequest()) {
    return GetObjectOutcome(
        OssError("ClientError:100002", "Disable all requests by upper."));
  }

  // sort
  std::sort(downloadedParts.begin(), downloadedParts.end(),
  ...
  if (!request_.RangeIsSet()) {
    if (client_->configuration().enableCrc64) {
      ...
      for (size_t i = 1; i < downloadedParts.size(); i++) {
        localCRC64 = CRC64::CombineCRC(localCRC64, downloadedParts[i].crc64,
                                       downloadedParts[i].size);
      }
      if (localCRC64 != meta.CRC64()) {
        return GetObjectOutcome(OssError(
            "CrcCheckError", "ResumableDownload object CRC checksum fail."));
      }
    }
    ...
    if (request_.RangeEnd() != -1) {
      ss << std::to_string(request_.RangeEnd()) << "/"
         << std::to_string(objectSize_);
    } else {
      ss << std::to_string(objectSize_ - 1) << "/"
         << std::to_string(objectSize_);
    }
    meta.HttpMetaData()["Content-Range"] = ss.str();
  }

  if (meta.HttpMetaData().find("x-oss-hash-crc64ecma-by-client") !=
      meta.HttpMetaData().end()) {
...
int ResumableDownloader::validateRecord() {
  ...
  if (request_.RangeIsSet()) {
    if (record.rangeStart != request_.RangeStart() ||
        record.rangeEnd != request_.RangeEnd()) {
...
int ResumableDownloader::loadRecord() {
  ...
  if (recordStream->is_open()) {
    ...
    if (root["rangeStart"] != Json::nullValue &&
        root["rangeEnd"] != Json::nullValue) {
      record_.rangeStart = root["rangeStart"].asInt64();
      record_.rangeEnd = root["rangeEnd"].asInt64();
    } else if (root["rangeStart"] == Json::nullValue &&
               root["rangeEnd"] == Json::nullValue) {
...
void ResumableDownloader::genRecordPath() {
  ...
  auto srcPath = ss.str();
  auto destPath = !request_.FilePathW().empty() ? toString(request_.FilePathW())
                                                : request_.FilePath();
  auto safeFileName =
      ComputeContentETag(srcPath) + "--" + ComputeContentETag(destPath);

  if (!request_.CheckpointDirW().empty()) {
    recordPathW_ =
        request_.CheckpointDirW() + WPATH_DELIMITER + toWString(safeFileName);
    ;
  } else {
...
int ResumableDownloader::getPartsToDownload(OssError &err,
                                            PartRecordList &partsToDownload) {
  UNUSED_PARAM(err);
...
void ResumableDownloader::DownloadPartProcessCallback(size_t increment,
                                                      int64_t transfered,
                                                      int64_t total,
                                                      void *userData) {
  UNUSED_PARAM(total);
  ...
  if (process.Handler) {
    process.Handler(increment, downloader->consumedSize_,
                    downloader->contentLength_, process.UserData);
  }
}
...
GetObjectOutcome
ResumableDownloader::GetObjectWrap(const GetObjectRequest &request) const {
